...
| Positron Assistant | R users who don't want to use VScode. Positron is a fork (copy) of VScode managed by Posit, the group who run RStudio. | - Possibly a bit easier to set-up R than with VScode<br>- Pros R friendly developer. Cons on version 0 so software will likely change | - New software<br>- Limited features to date |
...
There are many other (10s, 100s?) of coding assistants now available. Some prominent examples are Cursor and Windsurf. I haven't tried these, I understand they are similar to Github Copilot. Last time I checked none of them worked with Rstudio. We'll come back to Claude Code specifically in [Section 2](section-2-agents/01-agent-options.qmd), since it's really an agent rather than an inline-completion tool.
