...
Enable Next Edit Suggestions from the Copilot settings menu. Open a short R script that uses a variable with an uninformative name (e.g. `x` or `dat2`). Rename it in one place and watch Copilot suggest the same rename elsewhere in the file. Accept each suggestion with `tab`. Did it catch every occurrence, or did it miss any?
:::
