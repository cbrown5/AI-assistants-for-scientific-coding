...
The ecosystem of [agents implemented in R](https://luisdva.github.io/llmsr-book/r-pkgs.html#language-model-agents) is also growing. These R agents run from R, allowing you to integrate them into your own code. 

You should keep a markdown updated for continued agent use, to keep the agent on topic. You can use a `readme.md` for this purpose. A common standard is to use an `AGENTS.md` markdown, many agents are programmed to look for and use `AGENTS.md`. If there's out of date or inappropriate context in the readme (such as mentioning an R package you don't want to use), you'll find the agent defaults back to using the wrong tools. 
